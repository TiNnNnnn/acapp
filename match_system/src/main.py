...
from queue import Queue #线程安全同步队列
    ...
    def match_success(self,ps):
        ...
        for p in ps:
            async_to_sync(channel_layer.group_send)(
                room_name,
                {
                    'type': "group_send_event", #直接调用游戏server中的group_send_event函数
                    'event': "create_player",
                    'uuid':p.uuid,
                    'username':p.username,
                    'photo':p.photo,
...
def get_player_from_queue(): #从队列获取元素
    try:
...
if __name__ == '__main__':
    ...
    # You could do one of these for a multithreaded server # 多线程
    server = TServer.TThreadedServer( processor, transport, tfactory, pfactory)

    # server = TServer.TThreadPoolServer(
    #     processor, transport, tfactory, pfactory)

    Thread(target = worker , daemon=True).start() #daemon=true 杀死主线程后子线程关闭
